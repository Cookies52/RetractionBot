        ...
        for identifier in retracted_identifiers:
            ...
            for wp_page in page_list:
                ...
                for i, item in enumerate(raw_templates):
                    new_code = None
                    if i == len(raw_templates) - 1 or raw_templates[i+1].name.casefold().strip() not in (x.casefold() for x in template_map.values()):
                        ...
                        if item.name.lower() in [doi_field.lower(), "doi-inline"]:
                            ...
                            if item.has("1", ignore_empty=True):
                                record = database.retrieve_retracted_identifier(item.get("1").value.strip())
                        ...
                        if item.has(doi_field, ignore_empty=True):
                            record = database.retrieve_retracted_identifier(item.get(doi_field).value.strip())
                        elif item.has(pmid_field, ignore_empty=True):
                            record = database.retrieve_retracted_identifier(item.get(pmid_field).value.strip())
                        else:
                            continue
                        in_use = None
                        ...
                        if in_use is not None:
                            ...
                            intentional_field = field_map.get("intentional", "intentional")
                            if raw_templates[i+1].has(intentional_field, ignore_empty=True):
                                new_code.add(intentional_field, raw_templates[i+1].get(intentional_field).value.strip())
                ...
                if page_text != wp_page.text and bot_can_run:
                    ...
                    for x in changes:
                        database.log_retraction_edit(datetime.datetime.now(),
...
def process_item(record, template_map, field_map):
    ...
    if record.retraction_nature == "Retraction":
        logger.info("Generating Retraction template needed for DOI %s", record.original_doi)
        new_code = mwparserfromhell.nodes.template.Template(name=template_map.get("retracted", "Retracted"))
        ...
        new_code = mwparserfromhell.nodes.template.Template(name=template_map.get("expression of concern", "Expression of Concern"))
    elif record.retraction_nature == "Correction":
        logger.info("Generating Erratum template needed for DOI %s", record.original_doi)
        new_code = mwparserfromhell.nodes.template.Template(name=template_map.get("erratum", "Erratum"))
    else:
